...
export class TextFileSaveErrorHandler extends Disposable implements ISaveErrorHandler, IWorkbenchContribution {
	...
	onSaveError(error: unknown, model: ITextFileEditorModel): void {
		...
		// Any other save error
			...
			// Message
			if (canHandlePermissionOrLockedErrors && isWriteLocked) {
				if (triedToUnlock) {
					...
					message = localize('readonlySaveError', "Failed to save '{0}': File is locked. Select 'Overwrite' to attempt to unlock and write to it.", basename(resource));
				}
			} else if (canHandlePermissionOrLockedErrors && isPermissionDenied) {
